...
public class CustomMine implements Building {
	...
	public CustomMine(float cx, float cy) {
		this.cx = cx;
		this.cy = cy;
	...
	public void generate() {
		...
		if (this.output.getAllConnectedNodes().size() > 0) {
			BasicStorage storage = null;
			double dist = 0;
			boolean storageFound = false;
			for (Building b : GameScreen.buildingHandler.getAllBuildings()) {
				if (b instanceof BasicStorage) {
					storageFound = true;
					double newDist = this.output.getShortestDistanceTo(((BasicStorage) b).getInputNode());
					if (newDist < dist || dist == 0) {
						storage = (BasicStorage) b;
						dist = newDist;
					}
				}
			}
