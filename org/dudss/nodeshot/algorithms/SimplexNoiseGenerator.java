...
public class SimplexNoiseGenerator {
	...
	public float[][] generateOctavedSimplexNoise(int width, int height, int octaves, float roughness, float scale){
		...
		for (int x = 0; x < width; x++) {
			for (int y = 0; y < height; y++) {
				if (totalNoise[x][y] > max) max = totalNoise[x][y];
				if (totalNoise[x][y] < min) min = totalNoise[x][y];
			}
		}
	...
	public void randomizeMutatorTable() {
		...
		for(int i=0; i<512; i++)
		{
			perm[i]=p[Base.getRandomIntNumberInRange(1, 254) & 255];
			permMod12[i] = (short)(perm[i] % 12);
		}
	}
	
	//Copypasta (oh no), SimplexNoise formula
	...
	for(int i=0; i<512; i++)
	{
	//replaced "p[i]" with a random number
	perm[i]=p[Base.getRandomIntNumberInRange(1, 254) & 255];
	permMod12[i] = (short)(perm[i] % 12);
	}
	}
